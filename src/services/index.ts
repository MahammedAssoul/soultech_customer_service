 ...
 * or Supabase implementation. Switching `isMockService` in `appConfig.ts`
 * changes the entire data source.
 *
 * Dynamic imports are used so the Supabase client/library is only loaded when
 * mock mode is disabled — in mock mode no Supabase code is bundled or run.
 */
...
async function load<T extends Record<string, unknown>>(
  ...
  return isMockService ? mock() : supabase()
}
...
export const productService = {
  getProducts: (...args: Parameters<typeof import('./mock/mockProductService')['getProducts']>) =>
    load(
      ...
      () => import('./supabase/productService'),
